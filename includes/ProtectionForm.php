...
class ProtectionForm {
	...
	function __construct( Page $article ) {
		...
		$this->mTitleRestrictions = new TitleRestrictions( $this->mTitle );
		$this->mApplicableTypes = $this->mTitleRestrictions->getRestrictionTypes();
	...
	function loadData() {
		...
		$this->mCascade = $this->mTitleRestrictions->areRestrictionsCascading();
		...
		foreach( $this->mApplicableTypes as $action ) {
			...
			$this->mRestrictions[$action] = implode( '', $this->mTitleRestrictions->getRestrictions( $action ) );

			if ( !$this->mRestrictions[$action] ) {
				// No existing expiry
				$existingExpiry = '';
			} else {
				$existingExpiry = $this->mTitleRestrictions->getRestrictionExpiry( $action );
			}
	...
	function show( $err = null ) {
		...
		list( $cascadeSources, /* $restrictions */ ) = $this->mTitleRestrictions->getCascadeProtectionSources();
	...
	function save() {
		...
		$title_restrictions = new TitleRestrictions( $this->mArticle->getTitle() );
		$status = $title_restrictions->doUpdateRestrictions( $this->mRestrictions, $expiry, &$this->mCascade, $reasonstr, $wgUser );
	...
	function buildForm() {
		...
		if ( $wgUser->isAllowed( 'editinterface' ) ) {
			...
			$out .= '<p class="mw-conveniencelinks">' . $link . '</p>';
		}
